...
exports.run = async (client, msg, args) => {
  ...
  let pony = await client.userLib.request({url: `https://derpibooru.org/api/v1/json/images/${args[0].replace(/\D+/g, '')}`, json: true}).catch(() => 0);
  if (!pony || pony.image.hidden_from_users) {msg.channel.send(embedErr);return;}
  pony = pony.image;
