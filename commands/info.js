...
exports.run = (client, msg, args, Discord) => {
	...
	• Версия Node   :: ${process.version}\`\`\``)
				...
				.addField("Команда помощи", `**${prefix}help**`, true)
				.addField("Префикс", `**${client.userLib.config.prefix}**`, true);
			if (prefix != client.userLib.config.prefix) embed.addField("Префикс сервера", `**${prefix}**`, true);
			if (logchannel != 0) embed.addField("Канал логирования", `**<#${logchannel}>**`, true);
			embed.addField("Ссылки", `[Сайт](https://akin.server-discord.com)\n[Пригласить бота](https://discordapp.com/api/oauth2/authorize?client_id=${client.user.id}&permissions=8&scope=bot)\n[Главный сервер](https://discord.gg/bDum6sp)`, true)
